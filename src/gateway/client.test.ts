...
describe("GatewayClient close handling", () => {
  ...
  it("preserves pre-open transport errors as the close reason", () => {
    const onClose = vi.fn();
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://127.0.0.1:18789",
    ...
    ws.emitError(new Error("connect EPERM 127.0.0.1:18789"));
    ws.emitClose(1006, "");

    expect(onConnectError).toHaveBeenCalledWith(
      expect.objectContaining({ message: "connect EPERM 127.0.0.1:18789" }),
    );
    expect(onClose).toHaveBeenCalledWith(1006, "connect EPERM 127.0.0.1:18789");
    client.stop();
  });
