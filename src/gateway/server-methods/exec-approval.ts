...
import {
  createTantherExecApprovalAuthority,
  resolveTantherExecApprovalAuthority,
...
export function createExecApprovalHandlers(
  ...
  return {
    "exec.approval.request": async ({ params, respond, context, client }) => {
      ...
      if (typeof p.security !== "string" || !p.security.trim()) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "security is required"));
        return;
      }
      if (typeof p.ask !== "string" || !p.ask.trim()) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "ask is required"));
        return;
      }
      if (
        host === "node" &&
      ...
      try {
        record.authority = createTantherExecApprovalAuthority({
        ...
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.UNAVAILABLE, `tanther authority init failed: ${String(err)}`),
        );
        return;
      }
    ...
    "exec.approval.waitDecision": async ({ params, respond }) => {
      ...
      if (!decisionPromise) {
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.APPROVAL_EXPIRED, "approval expired or not found"),
        );
        return;
      }
    ...
    "exec.approval.resolve": async ({ params, respond, client, context }) => {
      ...
      if (resolvedId.kind === "none") {
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.APPROVAL_EXPIRED, "unknown or expired approval id"),
        );
        return;
      }
      ...
      if (snapshot?.authority) {
        try {
          snapshot.authority = resolveTantherExecApprovalAuthority({
          ...
          respond(
            false,
            undefined,
            errorShape(ErrorCodes.UNAVAILABLE, `tanther authority resolve failed: ${String(err)}`),
          );
          return;
        }
      }
      const ok = manager.resolve(approvalId);
      if (!ok) {
        respond(
          false,
          undefined,
