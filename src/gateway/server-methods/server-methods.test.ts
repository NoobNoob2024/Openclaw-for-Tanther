...
const tantherAuthorityMocks = vi.hoisted(() => ({
  createTantherExecApprovalAuthority: vi.fn((params: { approvalId: string }) => ({
    authority: "tanther" as const,
    commandId: `cmd-${params.approvalId}`,
    fingerprintId: `fp-${params.approvalId}`,
    snapshotHash: `hash-${params.approvalId}`,
  ...
  resolveTantherExecApprovalAuthority: vi.fn(
    (params: { authority: { currentState: string }; decision: string }) => ({
      ...params.authority,
      currentState: params.decision === "deny" ? "checked" : "approved",
      resolution: {
...
vi.mock("../../infra/tanther-exec-authority.js", () => tantherAuthorityMocks);
...
describe("exec approval handlers", () => {
  ...
  it("rejects approval requests without security", async () => {
    const { handlers, respond, context } = createExecApprovalFixture();
    await requestExecApproval({
      handlers,
      respond,
  ...
  it("rejects approval requests without ask", async () => {
    const { handlers, respond, context } = createExecApprovalFixture();
    await requestExecApproval({
      handlers,
      respond,
  ...
  it("broadcasts request + resolve", async () => {
    ...
    expect(tantherAuthorityMocks.createTantherExecApprovalAuthority).toHaveBeenCalled();
    expect(tantherAuthorityMocks.resolveTantherExecApprovalAuthority).toHaveBeenCalled();
  });

  it("fails request registration when tanther authority init fails", async () => {
    const { handlers, respond, context } = createExecApprovalFixture();
    tantherAuthorityMocks.createTantherExecApprovalAuthority.mockImplementationOnce(() => {
      throw new Error("tanther init exploded");
    });
    ...
    await requestExecApproval({
      ...
      respond,
      context,
      params: { id: "approval-init-fail", twoPhase: true },
    });
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        code: "UNAVAILABLE",
        message: expect.stringContaining("tanther authority init failed"),
      }),
    );
  });

  it("fails resolve when tanther authority resolve fails and keeps approval unresolved", async () => {
    ...
    const requestPromise = requestExecApproval({
      ...
      respond,
      context,
      params: { id: "approval-resolve-fail", twoPhase: true },
    });

    const requested = broadcasts.find((entry) => entry.event === "exec.approval.requested");
    const id = (requested?.payload as { id?: string })?.id ?? "";
    expect(id).toBe("approval-resolve-fail");

    tantherAuthorityMocks.resolveTantherExecApprovalAuthority.mockImplementationOnce(() => {
      throw new Error("tanther resolve exploded");
    });
    ...
    expect(resolveRespond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        code: "UNAVAILABLE",
        message: expect.stringContaining("tanther authority resolve failed"),
      }),
    );
    expect(broadcasts.some((entry) => entry.event === "exec.approval.resolved")).toBe(false);
    expect(manager.getSnapshot(id)?.authority?.resolution).toBeUndefined();
    expect(manager.expire(id, "test-cleanup")).toBe(true);
    await requestPromise;
  });
  ...
  it("does not reuse a resolved exact id as a prefix for another pending approval", () => {
    ...
    resolvedRecord.authority = tantherAuthorityMocks.resolveTantherExecApprovalAuthority({
      authority: tantherAuthorityMocks.createTantherExecApprovalAuthority({ approvalId: "abc" }),
      decision: "allow-once",
    });
    expect(manager.resolve("abc")).toBe(true);
  ...
  it("accepts unique short approval id prefixes", async () => {
    ...
    record.authority = tantherAuthorityMocks.createTantherExecApprovalAuthority({
    ...
    expect(manager.getSnapshot(record.id)?.authority?.resolution?.decision).toBe("allow-once");
  });
  ...
  it("resolves only the targeted approval id when multiple requests are pending", async () => {
    ...
    expect(resolveRespond).toHaveBeenCalledWith(true, { ok: true }, undefined);
    expect(manager.getSnapshot("approval-one")?.authority?.resolution?.decision).toBe("allow-once");
    expect(manager.getSnapshot("approval-two")?.authority?.resolution).toBeUndefined();
