...
function formatExecDeniedReason(reason: string): string {
  const trimmed = reason.trim();
  if (!trimmed) {
    return "";
  }
  const status = normalizeSurfaceAuthorityStatus({ message: trimmed });
...
export const handleNodeEvent = async (ctx: NodeEventContext, nodeId: string, evt: NodeEvent) => {
  switch (evt.event) {
    ...
    case "exec.denied": {
      ...
      const reason = formatExecDeniedReason(typeof obj.reason === "string" ? obj.reason : "");
