...
function expectedBuildSpawn() {
  return [
    process.execPath,
    path.join(path.dirname(fileURLToPath(import.meta.url)), "../../scripts/tsdown-build.mjs"),
    "--no-clean",
  ];
}
    ...
    async () => {
      await withTempDir(async (tmp) => {
        ...
        const spawn = (cmd: string, args: string[]) => {
          if (cmd === process.execPath && args[0] === expectedBuildSpawn()[1]) {
