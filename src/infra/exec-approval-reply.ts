...
import { APPROVE_USAGE_TEXT } from "./operator-approval-surface.js";
...
export type ExecApprovalReplyMetadata = {
  approvalId: string;
  approvalSlug: string;
  allowedDecisions?: readonly ExecApprovalTransportDecision[];
};
...
export function getExecApprovalReplyMetadata(
  ...
  const allowedDecisions = Array.isArray(record.allowedDecisions)
    ? record.allowedDecisions.filter(
        (value): value is ExecApprovalTransportDecision => isExecApprovalTransportDecision(value),
      )
    : undefined;
  return {
    approvalId,
  ...
  return {
    ...
    channelData: {
      execApproval: {
        ...
        allowedDecisions: EXEC_APPROVAL_ALLOWED_DECISIONS,
      },
    },
  };
}
