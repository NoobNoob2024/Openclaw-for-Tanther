...
import { resolveCommandTargetSecretLocally } from "./command-secret-local-resolution.js";
...
async function resolveCommandSecretRefsLocally(params: {
  ...
  for (const target of discoverConfigSecretTargetsByIds(sourceConfig, params.targetIds)) {
    ...
    await resolveCommandTargetSecretLocally({
      target,
      sourceConfig,
      resolvedConfig,
      env: context.env,
      ...
      activePaths,
      inactiveRefPaths,
      strict: enforcesResolvedSecrets(params.mode),
