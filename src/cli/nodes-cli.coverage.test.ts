...
describe("nodes-cli coverage", () => {
  ...
  it("fails closed when approval request times out even if askFallback=full", async () => {
    nodeExecApprovalsFile = {
      version: 1,
      defaults: {
        security: "full",
    ...
    callGateway.mockImplementation(async (opts: NodeInvokeCall) => {
      if (opts.method === "node.list") {
        return {
          nodes: [
            {
    ...
    await expect(runNodesCommand(["nodes", "run", "--node", "mac-1", "echo", "hi"])).rejects.toThrow(
      "__exit__:1",
    );
    expect(runtimeErrors.join("\n")).toContain("exec denied: approval timed out");
  });
