...
import { resolveExecApprovalsFromFile } from "../../infra/exec-approval-settings.js";
...
import { buildExecApprovalCliError } from "../exec-approval-outcome.js";
...
async function maybeRequestNodesRunApproval(params: {
  ...
  if (decision === "deny") {
    throw buildExecApprovalCliError("user-denied");
  }
  if (!decision) {
    throw buildExecApprovalCliError("approval-expired");
  }
      ...
      .action(async (command: string[], opts: NodesRunOpts) => {
        await runNodesCommand("run", async () => {
          ...
          if (approvals.hostSecurity === "deny") {
            throw buildExecApprovalCliError("security-deny");
          }
