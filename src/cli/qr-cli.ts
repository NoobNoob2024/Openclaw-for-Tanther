...
import { resolveGatewayAuthPasswordSecretRefValue } from "../gateway/gateway-auth-secret-resolution.js";
...
async function resolveLocalGatewayPasswordSecretIfNeeded(
  ...
  const resolvedPassword = await resolveGatewayAuthPasswordSecretRefValue({
    config: cfg,
    env: process.env,
  });
  if (!resolvedPassword) {
