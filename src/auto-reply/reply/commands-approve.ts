...
import {
  ...
  type ExecApprovalTransportDecision,
} from "../../infra/approval-transport-contract.js";
import {
  APPROVE_USAGE_TEXT,
  buildApproveForeignBotText,
  buildExecApprovalResolveFailureText,
  buildExecApprovalSubmittedText,
} from "../../infra/operator-approval-surface.js";
import {
...
const DECISION_ALIASES: Record<string, ExecApprovalTransportDecision> = {
  allow: "allow-once",
  once: "allow-once",
...
type ParsedApproveCommand =
  | { ok: true; id: string; decision: ExecApprovalTransportDecision }
  | { ok: false; error: string };

function parseApproveCommand(raw: string): ParsedApproveCommand | null {
  const trimmed = raw.trim();
  if (FOREIGN_COMMAND_MENTION_REGEX.test(trimmed)) {
    return { ok: false, error: buildApproveForeignBotText() };
  }
  ...
  if (isExecApprovalTransportDecision(DECISION_ALIASES[first])) {
    return {
      ok: true,
  ...
  if (isExecApprovalTransportDecision(DECISION_ALIASES[second])) {
    return {
      ok: true,
...
export const handleApproveCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  try {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: buildExecApprovalResolveFailureText(err),
      },
    };
  }
  ...
  return {
    shouldContinue: false,
    reply: { text: buildExecApprovalSubmittedText(parsed) },
  };
};
