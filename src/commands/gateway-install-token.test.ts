...
const resolveGatewayInstallTokenSecretBoundaryMock = vi.hoisted(() => vi.fn());
...
vi.mock("./gateway-install-token-secret.js", () => ({
  resolveGatewayInstallTokenSecretBoundary: resolveGatewayInstallTokenSecretBoundaryMock,
}));
...
describe("resolveGatewayInstallToken", () => {
  beforeEach(() => {
    ...
    resolveGatewayInstallTokenSecretBoundaryMock.mockResolvedValue({
      available: true,
      warning:
  ...
  it("returns unavailable reason when token SecretRef is unresolved in token mode", async () => {
    ...
    resolveGatewayInstallTokenSecretBoundaryMock.mockResolvedValue({
      available: false,
      unavailableReason: "gateway.auth.token SecretRef is configured but unresolved (missing env var).",
    });
  ...
  it("returns unavailable reason when token and password are both configured and mode is unset", async () => {
    ...
    expect(resolveGatewayInstallTokenSecretBoundaryMock).not.toHaveBeenCalled();
  });
