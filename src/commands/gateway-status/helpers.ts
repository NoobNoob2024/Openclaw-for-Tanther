...
import { resolveGatewayAuthSecretStatus } from "../../gateway/gateway-auth-secret-status.js";
...
export async function resolveAuthForTarget(
  ...
  const resolveToken = async (value: unknown, path: string): Promise<string | undefined> => {
    const tokenResolution = await resolveGatewayAuthSecretStatus({
      config: cfg,
      env: process.env,
      value,
      path,
  ...
  const resolvePassword = async (value: unknown, path: string): Promise<string | undefined> => {
    const passwordResolution = await resolveGatewayAuthSecretStatus({
      config: cfg,
      env: process.env,
      value,
      path,
