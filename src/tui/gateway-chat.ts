...
export async function resolveGatewayConnection(
  ...
  if (isRemoteMode) {
    const remoteToken = explicitAuth.token
      ...
      : await resolveGatewayAuthSecretStatus({
          value: remote?.token,
  ...
  if (gatewayAuthMode === "password") {
    ...
    if (!password) {
      throwGatewayAuthResolutionError(
        formatGatewayAuthSecretUnavailableReason({
          path: "gateway.auth.password",
          unresolvedRefReason: localPassword.unresolvedRefReason,
          fallbackMessage: "Missing gateway auth password.",
        }),
      );
    }
    return {
  ...
  const resolveToken = async () => {
    ...
    if (!token) {
      throwGatewayAuthResolutionError(
        formatGatewayAuthSecretUnavailableReason({
          path: "gateway.auth.token",
          unresolvedRefReason: localToken.unresolvedRefReason,
