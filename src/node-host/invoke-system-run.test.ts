...
const tantherRecordMocks = vi.hoisted(() => ({
  createTantherSystemRunAuthority: vi.fn((params: { runId: string }) => ({
    authority: "tanther" as const,
    commandId: `cmd-${params.runId}`,
    fingerprintId: `fp-${params.runId}`,
    snapshotHash: `hash-${params.runId}`,
    snapshotVersion: "openclaw-node",
    approvalScope: "system.run:node:node",
    requiredApproverClass: "none",
    currentState: "approved",
    guardDecision: "proceed",
    stages: [],
  })),
  recordTantherExecPostExecution: vi.fn(() => ({
    currentState: "recorded",
    receiptId: "receipt-1",
    evidenceIds: ["approval_request_log", "execution_receipt"],
    rollbackStatus: "eligible",
  })),
}));

vi.mock("../infra/tanther-exec-authority.js", () => tantherRecordMocks);
...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  it("records Tanther post-exec proof for approval-governed runs", async () => {
    tantherRecordMocks.createTantherSystemRunAuthority.mockClear();
    tantherRecordMocks.recordTantherExecPostExecution.mockClear();
    ...
    await handleSystemRunInvoke({
      ...
      isCmdExeInvocation: () => false,
      sanitizeEnv: () => undefined,
      runCommand: vi.fn(async () => createLocalRunResult()),
    ...
    expect(tantherRecordMocks.recordTantherExecPostExecution).toHaveBeenCalledWith(
      expect.objectContaining({
        approvalId: "approval-1",
        request: expect.objectContaining({
          host: "node",
    ...
    expect(tantherRecordMocks.createTantherSystemRunAuthority).not.toHaveBeenCalled();
  });

  it("creates Tanther authority for direct system.run and records post-exec proof", async () => {
    tantherRecordMocks.createTantherSystemRunAuthority.mockClear();
    tantherRecordMocks.recordTantherExecPostExecution.mockClear();
    ...
    expect(tantherRecordMocks.createTantherSystemRunAuthority).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("fails closed when Tanther post-exec recording fails", async () => {
    tantherRecordMocks.recordTantherExecPostExecution.mockImplementationOnce(() => {
    ...
    await handleSystemRunInvoke({
      ...
      params: {
        ...
        approvalAuthority: {
          authority: "tanther" as const,
          commandId: "cmd-approval-2",
          fingerprintId: "fp-approval-2",
          snapshotHash: "hash-approval-2",
          snapshotVersion: "openclaw-node",
          approvalScope: "system.run:node:node-1",
          requiredApproverClass: "operator",
          currentState: "approved",
    ...
    expect(sendInvokeResult).toHaveBeenCalledWith({
      ...
      error: {
        code: "UNAVAILABLE",
        message: "tanther post-exec record failed (runId=approval-2)",
