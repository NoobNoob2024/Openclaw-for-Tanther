...
import { resolveExecApprovals } from "../infra/exec-approval-settings.js";
...
import {
  createTantherSystemRunAuthority,
  recordTantherExecPostExecution,
...
import {
  buildSystemRunAuthorityFailure,
  buildSystemRunAuthoritySuccess,
...
type SystemRunParsePhase = {
  ...
  approvalAuthority: import("../infra/tanther-exec-authority.js").TantherExecApprovalAuthorityRecord | null;
  approvalRequest: import("../infra/exec-approvals.js").ExecApprovalRequestPayload | null;
...
async function sendSystemRunDenied(
  ...
  await opts.sendInvokeResult({
    ...buildSystemRunAuthorityFailure("UNAVAILABLE", params.message),
  });
}
...
async function sendSystemRunCompleted(
  ...
  phase: Pick<
    SystemRunParsePhase,
    "approvalAuthority" | "approvalRequest" | "runId" | "execution" | "commandText"
  >,
  result: ExecFinishedResult,
  payloadJSON: string,
) {
  if (phase.approvalAuthority && phase.approvalRequest) {
    try {
      recordTantherExecPostExecution({
        approvalId: phase.runId,
        authority: phase.approvalAuthority,
        request: phase.approvalRequest,
        runId: phase.runId,
        result: {
      ...
      logWarn(`tanther post-exec record failed (runId=${phase.runId}): ${String(err)}`);
      await opts.sendExecFinishedEvent({
        sessionKey: phase.execution.sessionKey,
        runId: phase.execution.runId,
        commandText: phase.execution.commandText,
        result,
        suppressNotifyOnExit: phase.execution.suppressNotifyOnExit,
      });
      await opts.sendInvokeResult({
        ...buildSystemRunAuthorityFailure(
          "UNAVAILABLE",
          `tanther post-exec record failed (runId=${phase.runId})`,
        ),
      });
      return;
    }
  ...
  await opts.sendInvokeResult(buildSystemRunAuthoritySuccess(payloadJSON));
}
...
async function parseSystemRunPhase(
  ...
  if (!command.ok) {
    await opts.sendInvokeResult({
      ...buildSystemRunAuthorityFailure("INVALID_REQUEST", command.message),
    });
    return null;
  }
  if (command.argv.length === 0) {
    await opts.sendInvokeResult({
      ...buildSystemRunAuthorityFailure("INVALID_REQUEST", "command required"),
    });
    return null;
  }
  ...
  if (opts.params.systemRunPlan !== undefined && !approvalPlan) {
    await opts.sendInvokeResult({
      ...buildSystemRunAuthorityFailure("INVALID_REQUEST", "systemRunPlan invalid"),
    });
    return null;
  }
  ...
  return {
    ...
    approvalAuthority: opts.params.approvalAuthority ?? null,
    approvalRequest: opts.params.approvalRequest ?? null,
    envOverrides,
...
async function evaluateSystemRunPolicyPhase(
  ...
  const approvalRequest =
    ...
    ({
      ...
      commandArgv: parsed.shellPayload === null ? parsed.argv : undefined,
      systemRunPlan: parsed.approvalPlan ?? undefined,
      cwd: parsed.cwd ?? null,
      security,
      ask,
      agentId: parsed.agentId ?? null,
      sessionKey: parsed.sessionKey ?? null,
    } satisfies import("../infra/exec-approvals.js").ExecApprovalRequestPayload);
  const approvalAuthority =
    parsed.approvalAuthority ??
    createTantherSystemRunAuthority({
      runId: parsed.runId,
...
async function executeSystemRunPhase(
  ...
  if (useMacAppExec) {
    ...
    if (!response) {
      ...
      await sendSystemRunCompleted(opts, phase, result, JSON.stringify(result));
      return;
    }
  }
