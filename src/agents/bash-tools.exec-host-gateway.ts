...
import { requiresExecApproval } from "../infra/exec-approval-settings.js";
...
export async function processGatewayAllowlist(
  ...
  if (hostSecurity === "allowlist" && analysisOk && allowlistSatisfied) {
    ...
    if (!enforced.ok || !enforced.command) {
      throw buildExecDeniedError({
        status: "allowlist-plan-unavailable",
  ...
  if (hostSecurity === "allowlist" && (!analysisOk || !allowlistSatisfied)) {
    throw buildExecDeniedError({ status: "allowlist-miss" });
  }
