...
import {
  resolveGatewayAuthPasswordSecretRefValue,
  resolveGatewayAuthTokenSecretRefValue,
} from "../gateway/gateway-auth-secret-resolution.js";
...
async function resolveGatewayTokenSecretRef(
  ...
  const token = await resolveGatewayAuthTokenSecretRefValue({
    config: cfg,
    env,
  });
  if (!token) {
...
async function resolveGatewayPasswordSecretRef(
  ...
  const password = await resolveGatewayAuthPasswordSecretRefValue({
    config: cfg,
    env,
  });
  if (!password) {
